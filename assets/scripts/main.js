...
function getIncomeStorage() {
    let incomeStorage;
    if (localStorage.getItem("incomeStorage") === null) {
      incomeStorage = [];
    } else {
      incomeStorage = JSON.parse(localStorage.getItem("incomeStorage"));
    }
...
// Add expenses to localstorage
  function addIncome(estimatedBudget) {
    const incomeStorage = getIncomeStorage();
    incomeStorage.push(estimatedBudget);
    localStorage.setItem("incomeStorage", JSON.stringify(incomeStorage));
}
  


let renderExpensesTable = () => {
  let expenses = getIncomeStorage();
